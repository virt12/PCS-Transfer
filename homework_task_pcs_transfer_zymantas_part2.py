...
cur=conn.cursor()
cur.execute("CREATE TABLE ex_logs23(Some_description VARCHAR, Execution_environment_description VARCHAR, Date_when_benchmark_has_been_performed date, Server_name_where_benchmark_has_been_performed  VARCHAR, Application_version varchar, Test_suite_version varchar, Method_API varchar, Method_name varchar, Parameters varchar,  Testcase_execution_time_in_seconds float  );")
...
if (len(file_path) ==0 or (not os.path.isfile(file_path))):
    ...
    with open(file_path, 'rt') as f:
        ...
        for i in rpc_calls: 
            ...
            cur.execute("INSERT INTO ex_logs23(Some_description, Execution_environment_description,Date_when_benchmark_has_been_performed, Server_name_where_benchmark_has_been_performed, Application_version, Test_suite_version,Method_API, Method_name, Parameters,Testcase_execution_time_in_seconds) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (description, environment, date_time, server, app_version, version, api_names, a, b,c ))
            conn.commit()
